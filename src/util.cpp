...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "pulsar.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
    {
        boost::filesystem::path ConfPath;
               ConfPath = GetDataDir() / "pulsar.conf";
               ...
               fprintf(ConfFile, "port=15798\n");
               fprintf(ConfFile, "rpcport=15799\n");
    ...
    {
        // Don't overwrite existing settings so command line settings override pulsar.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "pulsard.pid"));
...
// Format the subversion field according to BIP 14 spec (https://en.pulsar.it/wiki/BIP_0014)

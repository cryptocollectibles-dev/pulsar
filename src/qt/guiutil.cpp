...
#include "pulsaraddressvalidator.h"
...
{
    widget->setMaxLength(PulsarAddressValidator::MaxAddressLength);
    widget->setValidator(new PulsarAddressValidator(parent));
    widget->setFont(pulsarAddressFont());
}
...
bool parsePulsarURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // NovaCoin: check prefix
    if(uri.scheme() != QString("pulsar"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!PulsarUnits::parse(PulsarUnits::PLSR, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parsePulsarURI(QString uri, SendCoinsRecipient *out)
{
    // Convert pulsar:// to pulsar:
    //
    //    Cannot handle this later, because pulsar:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("pulsar://"))
    {
        uri.replace(0, 12, "pulsar:");
    }
    QUrl uriInstance(uri);
    return parsePulsarURI(uriInstance, out);
}
...
void openConfigfile()
{
    ...
    /* Open pulsar.conf with the associated application */
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a pulsar.desktop file to the autostart directory:
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Pulsar-Qt") + " " + tr("version") + " " +
...
void SetBlackThemeQSS(QApplication& app)
{
    app.setStyleSheet("QWidget        { background: rgb(41,44,48); }"
                      ...
                      "QPushButton    { background: rgb(63,67,72); color: rgb(255,255,255); }"
                      ...
                      "QMenuBar       { background: rgb(63,67,72); color: rgb(255,255,255); }"

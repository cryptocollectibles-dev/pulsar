 ...
 * The Pulsar Developers 2011-2012
 */
...
PulsarGUI::PulsarGUI(QWidget *parent):
    QMainWindow(parent),
    clientModel(0),
    walletModel(0),
    toolbar(0),
...
{
    resize(850+95, 550);
    setWindowTitle(tr("Pulsar") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/pulsar"));
...
void PulsarGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a Pulsar address"));
    ...
    aboutAction = new QAction(tr("&About Pulsar"), this);
    aboutAction->setToolTip(tr("Show information about Pulsar"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for Pulsar"));
    optionsAction->setMenuRole(QAction::PreferencesRole);
    toggleHideAction = new QAction(QIcon(":/icons/pulsar"), tr("&Show / Hide"), this);
...
void PulsarGUI::setClientModel(ClientModel *clientModel)
{
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/pulsar_testnet"));
            setWindowIcon(QIcon(":icons/pulsar_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/pulsar_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("Pulsar client") + QString(" ") + tr("[testnet]"));
...
void PulsarGUI::setWalletModel(WalletModel *walletModel)
{
...
void PulsarGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("Pulsar client"));
...
void PulsarGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Pulsar network", "", count));
}

void PulsarGUI::setNumBlocks(int count)
{
...
void PulsarGUI::message(const QString &title, const QString &message, bool modal, unsigned int style)
{
    QString strTitle = tr("Pulsar") + " - ";
...
void PulsarGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;
    ...
    QString strMessage = tr("This transaction is over the size limit. You can still send it for a fee of %1, "
        ...
        "Do you want to pay the fee?").arg(PulsarUnits::formatWithUnit(clientModel->getOptionsModel()->getDisplayUnit(), nFeeRequired));
...
void PulsarGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
        ...
        notificator->notify(Notificator::Information,
                            (amount)<0 ? tr("Sent transaction") :
                              ...
                              tr("Date: %1\n"
                                 ...
                                 "Address: %4\n")
                              .arg(date)
                              .arg(PulsarUnits::formatWithUnit(walletModel->getOptionsModel()->getDisplayUnit(), amount, true))
                              .arg(type)
                              .arg(address), icon);
    }
}
...
void PulsarGUI::gotoAddressBookPage()
{
...
void PulsarGUI::gotoBlockBrowser(QString transactionId)
{
...
void PulsarGUI::gotoSendCoinsPage()
{
...
void PulsarGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Pulsar address or malformed URI parameters."));
    }

    event->acceptProposedAction();
}

void PulsarGUI::handleURI(QString strURI)
{
    // URI has to be valid
...
void PulsarGUI::changePassphrase()
{
...
void PulsarGUI::updateStakingIcon()
{
    updateWeight();

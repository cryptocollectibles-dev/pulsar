...
{
    QString ret = QString("pulsar:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non PLSR unit input in lnReqAmount, we generate the URI with PLSR as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(PulsarUnits::format(PulsarUnits::PLSR, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {

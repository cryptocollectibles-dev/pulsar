...
Value getstakesubsidy(const Array& params, bool fHelp)
{
    ...
    return (uint64_t)GetProofOfStakeReward(0, pindexBest->nHeight);
}
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Pulsar is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Pulsar is downloading blocks...");
